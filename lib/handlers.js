...
handlers._tokens.put = (data, callback) => {
  ...
  if (id && extend) {
    ...
    _data.read('tokens', id, (err, tokenData) => {
      if (!err && tokenData) {
        // Check to the make sure the tokens isn't already expired
...
handlers._checks.post = (data, callback) => {
  ...
  const timeoutSeconds = typeof(data.payload.timeoutSeconds) === 'number' && data.payload.timeoutSeconds % 1 === 0 && data.payload.timeoutSeconds > 0 && data.payload.timeoutSeconds < 6 ? data.payload.timeoutSeconds : false
                ...
                _data.read('users', checkData.userPhone, (err, userData) => {
                  if (!err && userData) {
                    ...
                    if (checkPosition > -1) {
                      ...
                      callback(500, {error: 'Could not the find the check on the users object, so could not remove it!'})
                    }
                  } else {
